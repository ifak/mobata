...
KMscTimer::KMscTimer(KMscAnchor* from, KMscAnchor* to,
                     const QString& name, const double duration,
                     KMscSequence* parent)
  : KMscMessage(from, to,
                QString(QStringLiteral("timer '%1':\nafter %2s")).arg(name).arg(duration),
                parent),
...
qreal KMscTimer::updateGeometry(const qreal bottomSceneY,
