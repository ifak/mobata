...
QString placesString(const PlaceArcSet& placeArcs)
{
  ...
  int pos=0;
  foreach(const typename PlaceArcSet::value_type& placeArc, placeArcs)
  {
    if(!placeArc->place())
      continue;
...
void TransitionItem::removePrePlace(const PlaceItem *prePlace)
{
  ...
  foreach(PrePlaceArc const* prePlaceArc, this->prePlaceArcs())
  {
...
void TransitionItem::removePostPlace(const PlaceItem *postPlace)
{
  ...
  foreach(PostPlaceArc const* postPlaceArc, this->postPlaceArcs())
  {
